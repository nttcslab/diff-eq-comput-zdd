#ifndef COST_EXPWEIGHT_HPP
#define COST_EXPWEIGHT_HPP
...
void ExpCostW::setParams(const RealV& paramsval, bool gradcalc){
  pdim = paramsval.size();
  if(gradcalc){
    params.resize(pdim);
...
void ExpCostW::getGradParams(RealV& grad) const{
  grad.resize(pdim);
  adept::get_gradients(&params[0], pdim, &grad[0]);
}

void ExpCostW::gfunc(const aRealV& x, aRealV& res) const{
  size_t dim = x.size();
  assert(dim == pdim);
  res.resize(dim);
  for(size_t i=0; i<dim; ++i) res[i] = weights[i] * (10.0 * x[i] * exp(-params[i]) + 1.0);
}
void ExpCostW::gfunc(const RealV& x, RealV& res) const{
  size_t dim = x.size();
  assert(dim == pdim);
  res.resize(dim);
  for(size_t i=0; i<dim; ++i) res[i] = weights[i] * (10.0 * x[i] * exp(-params_na[i]) + 1.0);
}

aReal ExpCostW::fval(const aRealV& x) const{
  size_t dim = x.size();
  assert(dim == pdim);
  aRealV wf(dim);
  gfunc(x, wf);
...
Real ExpCostW::fval(const RealV& x) const{
  size_t dim = x.size();
  assert(dim == pdim);
  RealV wf(dim);
  gfunc(x, wf);
...
int ExpCostW::readWeightfromFile(const char *filename, const Graph& g){
  FILE *fp;
...
void ExpCostW::setDefaultWeight(size_t num_edge){
  weights.resize(num_edge);
  std::fill(weights.begin(), weights.end(), 1.0);
}

#endif // COST_EXPWEIGHT_HPP
