...
class RandomSimplex{
public:
  RandomSimplex(unsigned int _seed): engine(_seed) {};
  RandomSimplex(): engine(), dim(0) {};
  
  void setdim(int _dim){
    dim = _dim;
  }
  void setmult(double _mult){
    mult = _mult;
  }
  void rnd(std::vector<double>& res){
...
void print_usage(){
  ...
  fprintf(stderr, " -l [outer_loop_limit] : Set outer loop limit (default: 100000)\n");
  ...
  fprintf(stderr, " -r [seed_value]       : Set random seed value (default: random)\n");
}
...
int main(int argc, char **argv){
  ...
  while((opt = getopt(argc, argv, "g:o:z:w:e:t:i:s:l:nc3x:b:pqyr:")) != -1){
    switch (opt) {
  ...
  if(seed == -1) seed_num = (std::random_device())();
  else seed_num = static_cast<unsigned int>(seed);
  
  RandomSimplex RS(seed_num);
  RS.setdim(pdim);
  RS.setmult(1.0 * pdim);
  ...
  for(int t=0; t<outer_loop_num; ++t){
    ...
    if(printparams){
      for(const auto& v : paramsval) printf("%.10lf ", v-1.0);
      puts("");
    ...
    if(nowval < val){
      RS.rnd(paramsval);
      for(auto& v : paramsval) v += 1.0;
      nowval = 1e9;
    }else{
      Real newxave = 0.0;
      for(const auto& v : newx) newxave += v;
      newxave /= dim;
      for(size_t i=0; i<dim; ++i) paramsval[i] += del * (newx[i] - newxave);
      for(size_t i=0; i<pdim; ++i) paramsval[i] -= 1.0;
